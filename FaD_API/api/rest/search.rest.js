          ...
          if (accountNameCount === 0 && nonAccountNameCount === 0) { // 0 0
            ...
            if (
              ...
              if (sortingField !== 'distance') {
                ...
                Doctor.find(searchQueryGlobalObject).sort(sortObj).skip(skipBy).limit(limitSize).exec(function(err, doctors) {
                    ...
                    try {
                      ...
                      if (globalCountForAsyncCalls === doctors.length) {
                        ...
                        if (sortingField !== 'distance') {
						  ...
						  var sortObj = {};
						  sortObj[sortingField] = sortingOrderInNumbers;
                          Doctor.find(searchQueryGlobalObject).sort(sortObj).skip(skipByInner).limit(limit).exec(function(err, doctorsNonAccountName) {
                  /* globalResponse.send({
