...
var DoctorModel  =   dbConnection.model('Doctor');
var HospitalModel = dbConnection.model('Hospital');

DoctorModel.find({ hospitalAffillation : {$size  : 2  } }, function(err,result){
	if(!err){
                 
		  result.forEach(function(hosp,index){
          
          var hospArray = []; 
          hosp.hospitalAffillation.forEach(function(hospName,index){
              hospName = hospName.toUpperCase();
              
               HospitalModel.findOne({name:hospName},function(err,hos){
                 if(hos !=null)
                 hospArray.push(hos._id);
